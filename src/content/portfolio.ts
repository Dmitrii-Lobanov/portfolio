...
export const projects: Project[] = [
  ...
  },
  {
    ...
    status: "Live",
    summary:
      "An authenticated Kanban system designed around optimistic updates, rollback, per-task concurrency, stable API contracts, and production delivery.",
    context:
      "A task board looks simple until two clients update the same task, a request fails after an optimistic move, or authorization and persistence disagree.",
    role: "Product engineer",
    period: "2026",
    scale: "Web + API + database",
    outcome: "Fast interaction without silently sacrificing correctness.",
    nodes: [
      "React + Clerk",
      "Optimistic mutation",
      "Versioned REST",
    ...
    decisions: [
      {
        ...
        decision: "Optimistic mutation with scoped snapshots",
        tradeoff: "Rollback logic must remain explicit",
        result: "Immediate feedback with recoverable failures",
      },
      {
        constraint: "Concurrent edits can overwrite newer data",
        decision: "Version-based conflict detection",
        tradeoff: "Clients must handle rejected stale writes",
        result: "No silent last-write-wins data loss",
      },
      {
        constraint: "Frontend and API evolve independently",
        decision: "Shared TypeScript transport contracts",
        tradeoff: "Contract changes require coordination",
        result: "Stable behavior across persistence and UI",
      },
    ],
